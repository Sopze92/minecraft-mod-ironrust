...
public interface I_Rustable extends ChangeOverTimeBlock<I_Rustable.RustLevel> {
  Supplier<BiMap<Block, Block>> RUST_LEVEL_INCREASES = Suppliers.memoize(() -> (BiMap)ImmutableBiMap.builder()
  ...
  Supplier<BiMap<Block, Block>> RUST_LEVEL_DECREASES = Suppliers.memoize(() -> ((BiMap)RUST_LEVEL_INCREASES.get()).inverse());
  ...
  static Optional<Block> getDecreasedRustBlock(Block block) {
    return Optional.ofNullable((Block)((BiMap)RUST_LEVEL_DECREASES.get()).get(block));
  }

  static Block getUnaffectedRustBlock(Block block) {
    Block block2 = block;

    for(Block block3 = (Block)((BiMap)RUST_LEVEL_DECREASES.get()).get(block); block3 != null; block3 = (Block)((BiMap)RUST_LEVEL_DECREASES.get()).get(block3)) {
      block2 = block3;
    }
  ...
  static Optional<Block> getNext(Block block) {
    return Optional.ofNullable((Block)((BiMap)RUST_LEVEL_INCREASES.get()).get(block));
  }
